...
const TIER_LABEL: Record<MerchantTier, string> = { ALL: "全部", NORMAL: "普通", VIP: "VIP", STRATEGIC: "战略" };
const STATUS_BADGE: Record<FeeRule["status"], { label: string; badge: string; icon: React.ReactNode }> = {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-surface p-4 rounded-2xl border border-line/80 shadow-card">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-fg tracking-tight">费率规则引擎</h1>
          </div>
          <p className="text-xs text-fg-secondary mt-1 max-w-2xl">
            按渠道 / 币种 / 金额区间 / 商户分级配置多维费率规则，数字越小优先级越高，列表按优先级排序。
          </p>
        </div>
      ...
      <div className="bg-surface p-3 rounded-xl border border-line/80 shadow-card">
        <div className="flex items-center gap-1.5 text-xs font-semibold text-fg mb-2">
          <Calculator className="w-3.5 h-3.5 text-fg-tertiary" /> 费率试算工具
        </div>
        <div className="flex flex-col md:flex-row md:items-end gap-2 flex-wrap">
          <div className="w-full md:w-32"><label className="block text-[11px] text-fg-tertiary mb-1">渠道</label>
            <ShadcnSelect value={calcChannel} onValueChange={setCalcChannel} options={ALL_CHANNELS.map((c) => ({ value: c, label: CHANNEL_LABEL[c] }))} />
          </div>
          <div className="w-full md:w-28"><label className="block text-[11px] text-fg-tertiary mb-1">币种</label>
          ...
          <div className="w-full md:w-28"><label className="block text-[11px] text-fg-tertiary mb-1">金额</label>
          ...
          <div className="w-full md:w-32"><label className="block text-[11px] text-fg-tertiary mb-1">商户分级</label>
            <ShadcnSelect value={calcTier} onValueChange={setCalcTier} options={(["ALL", "NORMAL", "VIP", "STRATEGIC"] as MerchantTier[]).map((t) => ({ value: t, label: TIER_LABEL[t] }))} />
          </div>
          <button onClick={runCalc} className="inline-flex items-center gap-1.5 px-3 py-2 bg-primary hover:bg-primary-hover text-primary-foreground rounded-lg text-xs font-semibold shadow-card transition-colors">
            <Zap className="w-3.5 h-3.5" /> 计算
          </button>
        </div>
        {calcResult && (
          <div className="mt-3 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs">
            <div className="font-semibold text-emerald-800 mb-1.5">命中规则：{calcResult.rule.name}（优先级 {calcResult.rule.priority}）</div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-fg-secondary">
              <div>固定费 <span className="font-mono font-semibold text-fg float-right">{calcResult.fixed.toFixed(2)}</span></div>
              <div>百分比费 <span className="font-mono font-semibold text-fg float-right">{calcResult.percent.toFixed(2)}</span></div>
              <div>总手续费 <span className="font-mono font-semibold text-rose-600 float-right">{calcResult.fee.toFixed(2)}</span></div>
              <div>净入账 <span className="font-mono font-semibold text-emerald-700 float-right">{calcResult.net.toFixed(2)}</span></div>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-surface p-3 rounded-xl border border-line/80 shadow-card flex flex-col md:flex-row md:items-center justify-between gap-2 text-xs">
        ...
        <div className="relative w-full md:w-64">
          ...
          <input type="text" placeholder="搜索规则名称 / 币种..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-8 pr-3 py-1.5 bg-subtle border border-line rounded-lg text-xs" />
        </div>
      </div>
          ...
          <table className="min-w-[1100px] w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-subtle/90 border-b border-line text-fg-secondary font-semibold text-[11px]">
                ...
                <th className="py-2 px-3">适用渠道</th>
                <th className="py-2 px-3">币种</th>
                <th className="py-2 px-3">金额区间</th>
                <th className="py-2 px-3">商户分级</th>
                <th className="py-2 px-3 text-right">固定费</th>
                <th className="py-2 px-3 text-right">百分比</th>
                <th className="py-2 px-3 text-center">优先级</th>
                ...
                return (
                  <ContextMenu
                    ...
                    items={[
                      ...
                      { key: "toggle", label: r.status === "ENABLED" ? "停用" : "启用", icon: <RefreshCw className="w-3.5 h-3.5" />, danger: r.status === "ENABLED", onClick: () => toggleStatus(r) },
      ...
      <SideSheet
        ...
        description="规则按优先级（数字越小越先匹配）对渠道/币种/金额区间/分级做首命中匹配"
        icon={<SlidersHorizontal className="w-5 h-5 text-fg" />}
        widthClass="max-w-2xl max-md:max-w-none"
        footer={
          <>
            {editing && (
              <Popconfirm title="确认删除该费率规则？" description={`规则「${editing.name}」删除后不可恢复。`} confirmText="确认删除" onConfirm={() => { removeRule(editing); setFormOpen(false); }}>
      ...
      >
        <div className="space-y-3 text-xs">
          <div>
            ...
            <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className="w-full px-3 py-2 bg-subtle border border-line rounded-lg text-xs" placeholder="如：Stripe 标准信用卡费率" />
          </div>
          <div>
            <label className="block text-fg-secondary mb-1.5 font-medium">适用渠道（可多选）</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-fg-secondary mb-1.5 font-medium">适用币种</label>
            ...
            <div>
              <label className="block text-fg-secondary mb-1.5 font-medium">商户分级</label>
              <ShadcnSelect value={form.merchantTier} onValueChange={(v) => setForm((f) => ({ ...f, merchantTier: v as MerchantTier }))} options={(["ALL", "NORMAL", "VIP", "STRATEGIC"] as MerchantTier[]).map((t) => ({ value: t, label: TIER_LABEL[t] }))} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-fg-secondary mb-1.5 font-medium">金额下限</label>
            ...
            <div>
              <label className="block text-fg-secondary mb-1.5 font-medium">金额上限（留空为无上限）</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-fg-secondary mb-1.5 font-medium">固定费</label>
            ...
            <div>
              <label className="block text-fg-secondary mb-1.5 font-medium">百分比费率</label>
              <input type="number" step="0.1" value={form.percentFee} onChange={(e) => setForm((f) => ({ ...f, percentFee: e.target.value }))} className="w-full px-3 py-2 bg-subtle border border-line rounded-lg text-xs font-mono" placeholder="2.9 = 2.9%" />
            </div>
            <div>
              <label className="block text-fg-secondary mb-1.5 font-medium">优先级 (1-99)</label>
